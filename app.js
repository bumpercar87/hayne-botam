...
function App() {
  ...
  return React.createElement("div", { style:{maxWidth:800,margin:"0 auto",padding:"1rem 1rem 3rem"} },
    React.createElement(Header, { user, view, goList, onLogout: handleLogout }),
    ...
    !loading && view==="new" && React.createElement(NewSession, { onDone: goList }),
...
function Header({ user, view, goList, onLogout }) {
...
function NewSession({ onDone }) {
  ...
  const [imgLoading, setImgLoading] = useState(false); const [saving, setSaving] = useState(false);
    ...
    React.createElement("div", { style:{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:"1.25rem"} },
      ...
      React.createElement("div", { style:row },
        ...
        React.createElement("div", { style:{display:"flex",gap:8,marginBottom:8} },
          React.createElement("button", { onClick:()=>fileRef.current.click(), disabled:imgLoading, style:{fontSize:12,padding:"6px 12px",borderRadius:6,border:`1px solid ${C.border}`,background:C.bg} }, imgLoading?"🔍 인식 중...":"📷 보탐 참여자 스크린샷 첨부"),
          React.createElement("input", { ref:fileRef, type:"file", accept:"image/*", style:{display:"none"}, onChange:handleImg }),
        ),
        React.createElement("div", { style:{display:"flex",gap:8} },
          React.createElement("input", { value:memberInput, onChange:e=>setMemberInput(e.target.value), placeholder:"닉네임 직접 입력 (쉼표/엔터 구분)", style:{flex:1}, onKeyDown:e=>{if(e.key==="Enter"){addMembers(memberInput);setMemberInput("");}} }),
          React.createElement("button", { onClick:()=>{addMembers(memberInput);setMemberInput("");} }, "추가"),
        ),
        React.createElement("div", { style:{display:"flex",flexWrap:"wrap",gap:6,marginTop:8,minHeight:32} },
          ...members.map(m=>React.createElement("span",{key:m,style:{background:C.bg,borderRadius:6,padding:"3px 9px",fontSize:12,display:"flex",alignItems:"center",gap:4,border:`1px solid ${C.border}`}},
...
function SessionDetail({ session, onBack, onUpdate }) {
  ...
  const save = async (updated) => {
    setS(updated);
    await apiPost({ action:"updateSession", data:updated });
    onUpdate();
  };
  const togglePaid = (name) => {
    const payments = s.payments.map(p=>p.name===name?{...p,paid:!p.paid}:p);
    save({...s, payments, allPaid:payments.every(p=>p.paid)});
  };
